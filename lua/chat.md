This small plugin helps me build prompts to chat with LLMs, it composes a prompt from any file on the codebase and sends it to some llm providers, then streams the response on the editor. The thing is my typical workflow is i create a markdown file on the codebase and build the prompt there, then either use this plugin commands to send the prompt and see the response or take that prompt to any LLM provider web based chat.

I want to implement a new feature, where i can with a command open up a markdown scratchpad, to make the promp composing a bit more "formal" or practical.

This is the main projects set of functions:

<llm_context name="lazyllm.lua" path="/home/santiago/projects/lazyllm.nvim/lua/lazyllm.lua" filetype="lua">
...
M.apply_diff_blocks = diff_utils.apply_diff_block
...
Tell me if you need more context to create the feature, ive split up utils based on domain, this is my oilnvim tree:

/004 󰢱  diff_utils.lua
/007 󰢱  file_utils.lua
/008 󰢱  git_utils.lua
/001 󰢱  lazyllm.lua
/005 󰢱  promp_utils.lua
/006 󰢱  providers.lua
/002 󰢱  symbol_utils.lua
